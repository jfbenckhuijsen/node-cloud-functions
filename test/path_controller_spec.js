  ...
  describe('--> cors based requests', () => {
    it('should answer to a CORS options request', () => {
      ...
      expect(res.statusCode)
        .to
        .equal(status.NO_CONTENT);
      expect(res.setHeader)
        .to
        .have
        .been
        .calledWith('Access-Control-Allow-Origin', '*');
      expect(res.end)
        .to
        .have
        .been
    ...
    it('should add CORS headers to a regular request', () => {
      ...
      expect(res.setHeader.calledWith('Access-Control-Allow-Origin', '*'))
        .to
        .equal(true);
    });
  ...
  describe('--> caching headers request', () => {
    it('should handle non-cacheable data', () => {
      ...
      expect(res.end)
        .calledWith();
      expect(res.setHeader)
        .calledWith('Cache-Control', 'no-store,no-cache,max-age=0');
    });
    ...
    it('should handle revalidation of data', () => {
      ...
      expect(res.end)
        .calledWith();
      expect(res.setHeader)
        .calledWith('Cache-Control', 'private,no-cache,max-age=0');
    });
    ...
    it('should handle data visible for public proxies', () => {
      ...
      expect(res.end)
        .calledWith();
      expect(res.setHeader)
        .calledWith('Cache-Control', 'public,max-age=300');
    });
    ...
    it('should handle cache private to the end user', () => {
      ...
      expect(res.end)
        .calledWith();
      expect(res.setHeader)
        .calledWith('Cache-Control', 'private,max-age=300');
    });
    ...
    it('should handle override for shared data', () => {
      ...
      expect(res.end)
        .calledWith();
      expect(res.setHeader)
        .calledWith('Cache-Control', 'public,no-transform,max-age=300,s-maxage=600');
    });
    ...
    it('should handle settings for stale data', () => {
      ...
      expect(res.end)
        .calledWith();
      expect(res.setHeader)
        .calledWith('Cache-Control', 'private,stale-if-error=600,stale-while-revalidate=150,max-age=300');
    });
    ...
    it('should handle raw settings', () => {
      ...
      expect(res.end)
        .calledWith();
      expect(res.setHeader)
        .calledWith('Cache-Control', 'public,must-revalidate,max-age=5');
    });
  });
});
