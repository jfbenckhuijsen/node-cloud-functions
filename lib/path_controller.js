    ...
    this.compiledPaths = options.paths.map((pathDef) => {
        let result = {
            ...
            definition: {
                ...
                use : [].concat(options.use || []).concat(pathDef.use || []).concat(defaultErrorHandler),
        ...
        if (pathDef.auth || (options.auth !== undefined && options.auth != false)) {
            ...
            result.definition.use = result.definition.use.concat(middleware);
        }
...
PathController.prototype.toApiRequest = function(req) {
    ...
    if (apiRequests.length > 1) {
        let apiRequests = apiRequests.filter((apiRequest) => {
