...
var PathController = function(options) {
    ...
    this.compiledPaths = options.paths.map((pathDef) => {
        ...
        result.definition.use = result.definition.use || options.use || [];
        result.definition.method = Array.isArray(result.definition.method) ? result.definition.method : [result.definition.method];
    ...
    /*
    var corsPaths = this.compiledPaths.filter((pathDef) => {
        return pathDef.cors || options.enableCors;
    }).map((path) => {
        var result = {
            params: path.params,
            definition : {
...
PathController.prototype.toApiRequest = function(req) {
    ...
    console.log("Trying to find a path which matches the request path " + req.path);
    ...
    console.log("Found " + apiRequests.length + " paths matching the request");
