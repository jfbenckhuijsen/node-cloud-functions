...
let PathController = function(options, DEBUG) {
    ...
    this.compiledPaths = options.paths.map((pathDef) => {
        ...
        if (pathDef.cors || (options.cors !== undefined && options.cors != false)) {
            ...
            result.definition.cors = pathDef.cors || options.cors;
        }
        ...
        result.definition.use = result.definition.use.concat(PathController.prototype.defaultErrorHandler);
...
PathController.prototype.defaultErrorHandler = (err, req, res, next) => {
    this.DEBUG("Default error handler was called with error {0}", err);
